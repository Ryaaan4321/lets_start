...
void galat_Karam()
{
    ...
    while (deq.size() && deq.front() == 0)
        deq.pop_front();
    while (deq.size() && deq.back() == 0)
        deq.pop_back();

    if (deq.empty() || (*max_element(deq.begin(), deq.end()) == *min_element(deq.begin(), deq.end()) && *min_element(deq.begin(), deq.end()) > 0))
    {
        yes;
    }
    else
