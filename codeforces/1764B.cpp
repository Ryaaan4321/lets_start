...
int32_t main()
{
    w(t)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            cin >> a[i];
            g = __gcd(g, a[i]);
            /*one thing we know about this problem is we want the element that can  divide  all the elements than we can figure out 
            how many elements are still there as there ass a multiple of that element and how to get that is to divide the max element of that array */
        }
        cout << a[n-1] / g << "\n";
