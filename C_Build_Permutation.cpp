...
template <typename T>
inline auto sqr(T x) -> decltype(x * x) { return x * x; }
...
int isPerfectSquare(int num)
{
    ...
    {
        return num; // Return the original number if it's a perfect square
    }
    else
    {
        return -1; // Return a special value (-1 in this case) to indicate it's not a perfect square
    }
}
